...
NrSlBwpManagerUe::DoTransmitNrSlRlcPdu(const NrSlMacSapProvider::NrSlRlcPduParameters& params)
{
    ...
    NS_ABORT_MSG_IF(it == m_nrSlBwpLcMap.end(),
                    "could not find Sap for Sidelink BWP " << +params.bwpId);
    ...
    const NrSlMacSapProvider::NrSlReportBufferStatusParameters& params)
{
    ...
    NS_ABORT_MSG_IF(itQci == m_slLcToBearerMap.end(),
                    "could not find QCI for Sidelink LC id " << +slLcIdentifier.lcId);
    uint8_t bwpIndex = m_algorithm->GetBwpForEpsBearer(itQci->second);

    NS_LOG_DEBUG("BSR of size " << params.txQueueSize
                                << " from RLC for LCID = " << static_cast<uint16_t>(params.lcid)
                                << " traffic type " << m_slLcToBearerMap.at(slLcIdentifier)
                                << " reported to BwpId " << static_cast<uint16_t>(bwpIndex));
