...
NrSlUeMac::DoSlotIndication(const SfnSf& sfn)
{
    ...
    if (m_enableSensing)
    {    
        if (m_slTxPool == nullptr){ 
       
        NS_LOG_ERROR("m_slTxPool is null");

        } else if(m_nrSlUePhySapProvider == nullptr)  {

        NS_LOG_ERROR(" m_nrSlUePhySapProvider is null");
        }
    ...
    if (m_nrSlUeMacScheduler != nullptr)
    {
        m_nrSlUeMacScheduler->SchedNrSlTriggerReq(sfn);
    }
    else
    {
        NS_LOG_ERROR("m_nrSlUeMacScheduler is null");
        return;
    }
