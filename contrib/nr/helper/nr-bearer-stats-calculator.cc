...
NrBearerStatsCalculator::UlRxPdu (uint16_t cellId, uint64_t imsi, uint16_t rnti, uint8_t lcid, uint32_t packetSize,
                                      uint64_t delay, uint16_t txRnti)
{
  NS_LOG_FUNCTION (this);
...
void
NrBearerStatsCalculator::DlRxPdu (uint16_t cellId, uint64_t imsi, uint16_t rnti, uint8_t lcid, 
                                  uint32_t packetSize, uint64_t delay, uint16_t txRnti)
{
  NS_LOG_FUNCTION (this);
...
void
NrBearerStatsCalculator::UeRlcBufferSizeSl (uint16_t cellId, uint64_t imsi, uint16_t rnti, uint32_t srcLcId, uint32_t destLcId, uint32_t rlcBufferSize, uint32_t rlcMaxBufferSize)
{
  ...
  //   m_ueRlcBufferSizeFile << "rnti"  << "," << "srcLcId" << "," << "destLcId"  << "," <<  "RlcBufferSize" << "," << "RlcMaxBufferSize"<< "," << "Timestamp" << std::endl;
	// }
  // m_ueRlcBufferSizeFile << rnti << "," << srcLcId << "," << destLcId << "," << rlcBufferSize << "," << rlcMaxBufferSize << "," << Simulator::Now () << std::endl;
}
