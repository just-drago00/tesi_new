...
UeToUePktTxRxOutputStats::Save (const std::string txRx, const Address &localAddrs,
                                ...
                                uint32_t seq, Time txTime)
{
  m_pktCache.emplace_back (UePacketResultCache (Simulator::Now ().GetSeconds (),
                                                txRx, localAddrs,
                                                nodeId, imsi, pktSize,
                                                srcAddrs, dstAddrs,
                                                seq, txTime));
  ...
  for (const auto & v : m_pktCache)
    {
      ...
      ret = m_db->Bind (stmt, 11, v.txtimeSec);
      NS_ABORT_UNLESS (ret);
