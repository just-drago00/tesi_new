...
public:
  ...
  static void NotifySlDrbCreated (NrBearerStatsConnector* c, std::string context, uint64_t imsi, uint16_t cellId, uint16_t rnti, uint32_t srcL2Id, uint32_t dstL2Id, uint8_t lcId);
...
private:
  ...
  void ConnectTracesUeSl (std::string context, uint64_t imsi, uint16_t cellid, uint16_t rnti);
