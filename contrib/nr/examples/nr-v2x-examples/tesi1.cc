...
int main(int argc, char* argv[])
{
   ...
   double dataRateddcdo = 256.0; // 256 kbps
   double dataRateddcugv = 200.0; // 200 kbps
   double dataRatedoddc = 300.0 + 256.0; // 556 kbps
   double dataRatedodo = 128.0; // 128 kbps
   double dataRateugvddc = 32500.0 + 256.0; // 32.756 Mbps 
   double dataRateddcrsu = 128.0; // 128 kbps
   double dataRatersuddc = 25256.0; // 25,256 Mbps
   double dataRateddccav = 128.0; // 200 kbps
   ...
   std::string exampleName = "prova_2cav20sec";
   ...
   cmd.AddValue("numerologyBwpSl",
                ...
                numerologyBwpSl);
   cmd.AddValue("numerology","The numerology to be used in fr1 bandwidth part",
                   numerology);    
   ...
   if(enablemoregnb){
       if(distance < 751){
       ...
       } else if (distance > 750 && distance < 1501){
           ...
           positionAlloc2->Add(Vector(distance/2 * (1 + cos(M_PI/3)) , distance / 2 * sin(M_PI/3), gNBheight));
           mobility2.SetPositionAllocator(positionAlloc2);
           mobility2.Install(gnbContainer.Get(1));
           MobilityHelper mobility3;
           mobility3.SetMobilityModel("ns3::ConstantPositionMobilityModel");
           Ptr<ListPositionAllocator> positionAlloc3 = CreateObject<ListPositionAllocator>();
           positionAlloc3->Add(Vector(- ( distance / 2 * (1+cos(M_PI/3)) ) , distance / 2 * sin(M_PI/3), gNBheight));
           mobility3.SetPositionAllocator(positionAlloc3);
           mobility3.Install(gnbContainer.Get(2));
           MobilityHelper mobility4;
           mobility4.SetMobilityModel("ns3::ConstantPositionMobilityModel");
           Ptr<ListPositionAllocator> positionAlloc4 = CreateObject<ListPositionAllocator>();
           positionAlloc4->Add(Vector(0.0, distance / 2 * ( sin(4*M_PI/3) + sin(5*M_PI/3) ), gNBheight));
           mobility4.SetPositionAllocator(positionAlloc4);
           mobility4.Install(gnbContainer.Get(3));
       } else {
           gnbContainer.Create(7);
           MobilityHelper mobility;
           mobility.SetMobilityModel("ns3::ConstantPositionMobilityModel");
           Ptr<ListPositionAllocator> positionAlloc = CreateObject<ListPositionAllocator>();
           positionAlloc->Add(Vector(0.0, 0.0, gNBheight));
           mobility.SetPositionAllocator(positionAlloc);
           mobility.Install(gnbContainer.Get(0));
           for(int i = 1,k=0; i < 7; i++,k++){
               MobilityHelper mobility2;
               mobility2.SetMobilityModel("ns3::ConstantPositionMobilityModel");
               Ptr<ListPositionAllocator> positionAlloc2 = CreateObject<ListPositionAllocator>();
               positionAlloc2->Add(Vector((distance - 750) * cos(k*M_PI/3) , (distance - 750) * sin(k*M_PI/3), gNBheight));
               mobility2.SetPositionAllocator(positionAlloc2);
               mobility2.Install(gnbContainer.Get(i));
           } 
       }
   } else {
       gnbContainer.Create(1);
   ...
   {
       nrHelper->GetGnbPhy(gNBNetDev.Get(i), 0)->SetAttribute("Pattern", StringValue(tddgnbPattern));
   }
   ...
   if (!useIPv6)
   {
       ...
       for (uint32_t i = 0; i < ueSlIpIface.GetN(); ++i)
       {
           ...
           if (i % numCAVsPerSector == 0) {   
               ...
               Address tempaddress = InetSocketAddress(ueSlIpIface.GetAddress(i), port11); 
